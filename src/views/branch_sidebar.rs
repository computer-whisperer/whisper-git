...
use crate::ui::text_util::{clamp_scroll, truncate_to_width};
use crate::ui::widget::{
...
impl BranchSidebar {
    ...
    pub fn set_branch_data(
        ...
        for remote_name in all_remote_names {
            self.remote_branches.entry(remote_name.clone()).or_default();
        }
        ...
        for wt in worktrees {
            if !wt.branch.is_empty() {
                self.branch_worktree_map
                    .insert(wt.branch.clone(), wt.name.clone());
            }
    ...
    fn build_visible_items(&mut self) {
        ...
        if !filtering || !local_filtered.is_empty() {
            ...
            if !self.local_collapsed {
                for branch in &local_filtered {
                    self.visible_items
                        .push(SidebarItem::LocalBranch(branch.clone()));
                }
            }
        }
        ...
        if !filtering || remote_has_matches {
            self.visible_items
                .push(SidebarItem::SectionHeader("REMOTE"));
            if !self.remote_collapsed {
                for (remote_name, branches) in &remote_filtered {
                    self.visible_items
                        .push(SidebarItem::RemoteHeader(remote_name.clone()));
                    if !self.collapsed_remotes.contains(remote_name) {
                        for branch in branches {
                            self.visible_items.push(SidebarItem::RemoteBranch(
                                remote_name.clone(),
                                branch.clone(),
                            ));
                        }
        ...
        if !self.stashes.is_empty() {
            let stash_filtered: Vec<StashEntry> = if filtering {
                self.stashes
                    .iter()
                    .filter(|s| self.matches_filter(&s.message))
                    .cloned()
                    .collect()
            } else {
                self.stashes.clone()
            ...
            if !filtering || !stash_filtered.is_empty() {
                self.visible_items
                    .push(SidebarItem::SectionHeader("STASHES"));
                if !self.stashes_collapsed {
                    for stash in &stash_filtered {
                        self.visible_items
                            .push(SidebarItem::StashEntry(stash.index));
                    }
                }
            }
        }
    ...
    fn move_focus(&mut self, delta: i32) {
        ...
        while idx >= 0 && (idx as usize) < len {
            if matches!(
                &self.visible_items[idx as usize],
            ...
            && let Some(item) = self.visible_items.get(idx)
        {
            match item {
                ...
                SidebarItem::RemoteBranch(remote, branch) => {
                    self.pending_action = Some(SidebarAction::CheckoutRemote(
                        remote.clone(),
                        branch.clone(),
                    ));
                }
        ...
        match &self.visible_items[idx] {
            SidebarItem::LocalBranch(name) => {
                ...
                if !effective.is_empty() && effective != name {
                    items.push(MenuItem::separator());
                    items.push(MenuItem::new(
                        format!("Merge into '{}'", effective),
                        "merge",
                    ));
                    items.push(MenuItem::new(
                        format!("Rebase '{}' onto", effective),
                        "rebase",
                    ));
                }
            ...
            SidebarItem::RemoteBranch(remote, branch) => {
                ...
                let mut items =
                    vec![MenuItem::new("Checkout", "checkout_remote").with_shortcut("Enter")];
            ...
            SidebarItem::Tag(name) => {
                let mut items = vec![MenuItem::new("Delete Tag", "delete_tag")];
                for item in &mut items {
                    if !item.is_separator {
                        item.action_id = format!("{}:{}", item.action_id, name);
                    }
            ...
            SidebarItem::SectionHeader("REMOTE") => Some(vec![
    ...
    pub fn handle_event(&mut self, event: &InputEvent, bounds: Rect) -> EventResponse {
            ...
            .is_consumed()
        {
            if let Some(ScrollAction::ScrollTo(ratio)) = self.scrollbar.take_action() {
                ...
                self.scroll_offset =
                    clamp_scroll(ratio * max_scroll, self.content_height, bounds.height);
            }
            return EventResponse::Consumed;
        }
    ...
    fn build_filtered_data(&self) -> FilteredData {
        ...
        let remotes: Vec<(String, Vec<String>)> = remote_names_sorted
            .iter()
            .filter_map(|rn| {
                let branches: Vec<String> = if filtering {
                    self.remote_branches[rn]
                        .iter()
    ...
    fn layout_filter_bar(
        ...
        if self.filter_focused && self.filter_cursor_visible {
            let cursor_x =
                input_x + text_renderer.measure_text(&self.filter_query[..self.filter_cursor]);
            ...
            for branch in filtered_local {
                ...
                if visible {
                    ...
                    let in_other_worktree =
                        !is_current && self.branch_worktree_map.contains_key(branch);
                    let is_focused = self.focused && self.focused_index == Some(item_idx);
                    let is_hovered = self.hovered_index == Some(item_idx);
                    ...
                    if is_hovered && !is_focused {
                        let highlight_rect =
                            Rect::new(params.inner.x, y, params.inner.width, params.line_height);
                        output.spline_vertices.extend(create_rect_vertices(
                            &highlight_rect,
                            theme::SURFACE_HOVER.with_alpha(0.3).to_array(),
                        ));
                    ...
                    if is_current {
                        ...
                        output
                            .spline_vertices
                            .extend(create_rect_vertices(&stripe_rect, theme::ACCENT.to_array()));
                    ...
                    let icon = if is_current || in_other_worktree {
                        "\u{25CF}"
                    } else {
                        "\u{25CB}"
                    }; // ● / ○
                    ...
                    let upstream_label = self
                        .upstream_map
                        .get(branch.as_str())
                        .map(|u| format!("\u{2192} {}", u));
                    let upstream_width = upstream_label
                        .as_ref()
                        .map(|label| {
                            params
                                .text_renderer
                                .measure_text_scaled(label, upstream_scale)
                                + 6.0
                        })
                        .unwrap_or(0.0);
                    ...
                    let wt_label_width = wt_label
                        .as_ref()
                        .map(|label| {
                            params
                                .text_renderer
                                .measure_text_scaled(label, wt_label_scale)
                                + 6.0
                        })
                        .unwrap_or(0.0);
                    ...
                    let name_max_width =
                        params.inner.width - params.indent - icon_width - right_reserved;
                    let display_name =
                        truncate_to_width(branch, params.text_renderer, name_max_width);
                    if is_current {
                    ...
                    if let Some(label) = &upstream_label {
                        ...
                        output
                            .text_vertices
                            .extend(params.text_renderer.layout_text_scaled(
                                label,
                                right_x,
                                y + 2.0 + y_offset,
                                theme::TEXT_MUTED.to_array(),
                    ...
                    if let Some(wt_text) = &wt_label {
                        let wt_w = params
                            .text_renderer
                            .measure_text_scaled(wt_text, wt_label_scale);
                        right_x -= wt_w;
                    ...
                    if branches.is_empty() {
                        ...
                        for branch in branches {
                            ...
                            if visible {
                                ...
                                let icon_width =
                                    params.text_renderer.measure_text("\u{25CB}") + 4.0;
                                let display_name = truncate_to_width(
                                    branch,
                                    params.text_renderer,
                                    params.inner.width - params.indent * 2.0 - icon_width,
                                );
                                output
                                    .text_vertices
                                    .extend(params.text_renderer.layout_text(
            ...
            for tag in filtered_tags {
                ...
                if visible {
                    ...
                    let display_name = truncate_to_width(
                        tag,
                        params.text_renderer,
                        params.inner.width - params.indent - icon_width,
                    );
                    output
                        .text_vertices
                        .extend(params.text_renderer.layout_text(
            ...
            for (stash_index, stash_msg, stash_time) in filtered_stashes {
                ...
                if visible {
                    ...
                    let display_name = truncate_to_width(
                        &label,
                        params.text_renderer,
                        params.inner.width - params.indent - icon_width - time_width,
                    );
                    output
                        .text_vertices
                        .extend(params.text_renderer.layout_text(
                    ...
                    if !time_str.is_empty() {
                        let time_x =
                            params.inner.right() - params.text_renderer.measure_text(&time_str);
                        output
                            .text_vertices
                            .extend(params.text_renderer.layout_text(
                                &time_str,
    ...
    pub fn layout(
        ...
        if show_local {
            (y, item_idx) = self.layout_local_section(
                &params,
                bold_renderer,
                &mut output,
                &data.local,
        ...
        if show_remote {
            (y, item_idx) = self.layout_remote_section(
                &params,
                bold_renderer,
                &mut output,
                &data.remotes,
        ...
        if show_tags {
            (y, item_idx) = self.layout_tags_section(
                &params,
                bold_renderer,
                &mut output,
                &data.tags,
        ...
        if show_stashes {
            y += section_gap;
            (y, item_idx) = self.layout_stashes_section(
                &params,
                bold_renderer,
                &mut output,
                &data.stashes,
        ...
        self.scrollbar
            .set_content(total_items, visible_items, scroll_offset_items);
        ...
        let filter_cover = Rect::new(bounds.x, bounds.y, bounds.width, padding + filter_bar_h);
        output.spline_vertices.extend(create_rect_vertices(
            &filter_cover,
            theme::PANEL_SIDEBAR.to_array(),
        ));
